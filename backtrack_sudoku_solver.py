...
# Function to print the board in a cleaner format, this one more fancy than the former. This version assumes the board is already complete.
def print_board(bo):
    for i in range(len(bo)):
        for j in range(len(bo[i])):
            if (j+1) % 3 == 0:
...
# Formats the board back. This is used only for the web version.
def format_back(bo):
    board = []
    for i in range(9):
        board.append(get_row(bo, i))
    return board
